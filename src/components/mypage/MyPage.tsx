...
import { listViewerProgress } from '../../lib/localUser';
  ...
  useEffect(() => {
    const loadLibraryData = async () => {
      try {
        ...
        const viewerProgress = listViewerProgress();
        ...
        viewerProgress.forEach((entry) => {
          if (seenPublicationIds.has(entry.publicationId)) {
            return;
          }
          ...
          progressData.push({
            ...
            progress: Math.round((entry.progress.progressRatio || 0) * 100),
            lastRead: new Date(entry.progress.updatedAt),
          });
          seenPublicationIds.add(entry.publicationId);
        });

...
import { ContributionItem, PresentationTrackOption, TextRole } from '@/types/publishing';
...
interface SpeakerContributionPanelProps {
  ...
  onStartEditSlot: (slotKey: string, value: string) => void;
    ...
    <section className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500">PDF 생성 순서와 동일합니다.</p>
        </div>
      ...
      <div className="space-y-3">
        {contributions.map((contribution, index) => {
          ...
          return (
            <div
              key={contribution.id}
              className={`rounded-2xl border px-3 py-3 ${isActive ? 'border-slate-900 bg-white' : 'border-slate-200 bg-white/80'}`}
            >
              <button
                type="button"
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs uppercase tracking-[0.18em] text-slate-400">#{contribution.order}</p>
                    {showPresentationTracks ? (
                      <p className="mt-1 text-xs font-semibold text-sky-700">
                        {contribution.presentationCode || '번호 미지정'}
                      </p>
                    ) : null}
                    <p className="mt-1 text-sm font-semibold text-slate-800">{contribution.title}</p>
                    <p className="mt-1 text-xs text-slate-500">{contribution.track || contribution.sourceFileName || '트랙 정보 없음'}</p>
                  </div>
      ...
      <section className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-slate-500">{selectedContribution.title}</p>
          </div>
          <div className="flex items-center gap-2">
                ...
                {isEditing ? (
                  <>
                    <textarea
                      ...
                      rows={slot.slotKey.startsWith('body') ? 10 : 4}
                  ...
                  <>
                    ...
                    <div className="mt-3 flex gap-2">
                      <button
                        type="button"
                        onClick={() => onStartEditSlot(slot.slotKey, slot.text)}
                        className="rounded-full bg-slate-900 px-4 py-2 text-xs font-semibold text-white"
                      >
                        슬롯 수정
                      </button>
